 ...
 */
public final class EsFuzzyTermsEnum extends BaseTermsEnum {
     ...
     */
    public EsFuzzyTermsEnum(Terms terms, Term term, int maxEdits, int prefixLength, boolean transpositions) throws IOException {
        this(terms, new AttributeSource(), term, () -> new EsFuzzyAutomatonBuilder(term.text(), maxEdits, prefixLength, transpositions));
    }
     ...
     * @throws IOException if there is a low-level IO error
     */
    EsFuzzyTermsEnum(Terms terms, AttributeSource atts, Term term, int maxEdits, int prefixLength, boolean transpositions)
        throws IOException {
        this(terms, atts, term, () -> new EsFuzzyAutomatonBuilder(term.text(), maxEdits, prefixLength, transpositions));
    }

    private EsFuzzyTermsEnum(Terms terms, AttributeSource atts, Term term, Supplier<EsFuzzyAutomatonBuilder> automatonBuilder)
        throws IOException {
     ...
     *
     * <p>Package-private so that {@link
     * EsFuzzyQuery#computeAutomataRamBytes(AttributeSource)} can reuse the same sharing mechanism.
     */
    interface AutomatonAttribute extends Attribute {
        ...
        void init(Supplier<EsFuzzyAutomatonBuilder> builder);
    }
    ...
    static class AutomatonAttributeImpl extends AttributeImpl implements AutomatonAttribute {
        ...
        @Override
        public void init(Supplier<EsFuzzyAutomatonBuilder> supplier) {
            if (automata != null) {
                return;
            }
            EsFuzzyAutomatonBuilder builder = supplier.get();
